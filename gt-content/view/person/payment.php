	...
	<div class="container-fluid">
		...
		if(isset($_POST['set_sharj'])){
			...
			if(isset($_POST['p_sharj']) && $_POST['p_sharj']!=""){
				$ps = $_POST['p_sharj'];
				ex_query("update person set p_sharj = $ps where p_id = $p_id");				
			}
			...
			<div class="col-md-6 text-left">
				<form action="" method="get">
					<select style="width: 70%; float: right;" name="p_id" class="select2 form-control">
						...
						<?php
						$items = get_select_query("select * from person where a_id = $a_id");
		...
		<div class="row">
							...
							<form action="" method="post">
									...
									<div class="col-md-4">
										<div class="alert alert-warning">
											...
											<select name="p_pack" id="pk_id" class="form-control">
												...
												<?php
												$res = get_select_query("select * from package where a_id = $a_id");
												if(count($res)){
													foreach($res as $row){
													?>
								...
								<div class="row">	
									<div class="col-md-4">
										...
										<input id="bp_time" name="p_sharj" type="text" class="form-control" placeholder="میزان دقیقه">
									</div>
									<div class="col-md-4">
			...
			<div class="col-md-3 text-center">
				...
				if(isset($_GET['p_id'])){
					$p_id = $_GET['p_id'];
					$t = get_person_status($p_id);
					echo "<div class='alert alert-warning'>شارژ " . per_number(convert_minute($sharj)) . " ساعت تا " . per_number($expire) . "</div>";
				}
				?>
			</div>
							...
							if(count($res)>0){
								foreach($res as $row){ ?>
										...
										?>
											...
											<td><?php echo per_number(number_format($row['pa_offer'])); ?> تومان</td>
								...
								<tr style="font-size: 20px;">
									...
									<th>جمع تخفیف: </th>
									<td><?php echo per_number(number_format($o)); ?> تومان</td>
								</tr>
							<?php
							} else { ?>
