			...
			<div class="col-md-12">
				...
				if(isset($_POST['send_sms'])){
					...
					<?php
						...
						$sms->set_log("anniversary", $sl_bulk, $sms_mobile, $sms_text);
					?>
					...
					<div class="col-12">
						<div class="card">
							<div class="card-header">
								<h3 class="card-title">تبریک سالگرد ازدواج</h3>
								<div class="card-tools">
									<form action="" method="get">
										<select name="p_m_date" onchange="this.form.submit()">
											<option value="<?php echo jdate('-m-d'); ?>">امروز</option>
											<option <?php if(isset($_GET['p_m_date']) && $_GET['p_m_date'] == '-01-') echo 'selected'; ?> value="-01-">فروردین</option>
											<option <?php if(isset($_GET['p_m_date']) && $_GET['p_m_date'] == '-02-') echo 'selected'; ?> value="-02-">اردیبهشت</option>
											<option <?php if(isset($_GET['p_m_date']) && $_GET['p_m_date'] == '-03-') echo 'selected'; ?> value="-03-">خرداد</option>
											<option <?php if(isset($_GET['p_m_date']) && $_GET['p_m_date'] == '-04-') echo 'selected'; ?> value="-04-">تیر</option>
											<option <?php if(isset($_GET['p_m_date']) && $_GET['p_m_date'] == '-05-') echo 'selected'; ?> value="-05-">مرداد</option>
											<option <?php if(isset($_GET['p_m_date']) && $_GET['p_m_date'] == '-06-') echo 'selected'; ?> value="-06-">شهریور</option>
											<option <?php if(isset($_GET['p_m_date']) && $_GET['p_m_date'] == '-07-') echo 'selected'; ?> value="-07-">مهر</option>
											<option <?php if(isset($_GET['p_m_date']) && $_GET['p_m_date'] == '-08-') echo 'selected'; ?>value="-08-">آبان</option>
											<option <?php if(isset($_GET['p_m_date']) && $_GET['p_m_date'] == '-09-') echo 'selected'; ?> value="-09-">آذر</option>
											<option <?php if(isset($_GET['p_m_date']) && $_GET['p_m_date'] == '-10-') echo 'selected'; ?> value="-10-">دی</option>
											<option <?php if(isset($_GET['p_m_date']) && $_GET['p_m_date'] == '-11-') echo 'selected'; ?> value="-11-">بهمن</option>
											<option <?php if(isset($_GET['p_m_date']) && $_GET['p_m_date'] == '-12-') echo 'selected'; ?> value="-12-">اسفند</option>
										</select>
									</form>
								</div>
							</div>
							<div class="card-body table-responsive p-0">		
								<form method="post" action="" class="form" onSubmit="if(!confirm('آیا از ارسال این پیامک اطمینان دارید؟')){return false;}">
									<table class="table table-striped">
										...
										<?php
										$i = 1;
										if(isset($_GET['p_m_date'])) {	
											$p_m_date = $_GET['p_m_date'];
										} else {
											$p_m_date = jdate('-m-d');
										}
										$sql = "select * from person inner join person_meta on person.p_id = person_meta.p_id where person_meta.pm_meta = 'marry_date' and person_meta.pm_value like '%$p_m_date%' order by person.p_id desc";
										...
										if(count($res) > 0) {
																...
																<div class="modal-dialog">
																	<div class="modal-content">
																		...
																		<div class="modal-body">
																			...
																			$sql_last = $db->get_select_query("select * from sms_log where sl_type = 'anniversary' and sl_rcpts = '$p_mobile'");
																			if(count($sql_last) > 0) {
																				?>
											...
											<tr>
												<td colspan="13" class="text-center">هیچ سالگرد ازدواجی نداریم!</td>
											</tr>
											<?php
											} ?>
									</table>
